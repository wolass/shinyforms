...
saveDataRobj <- function(data, storage) { #Write as R object
  fileName <- paste0(
    paste(
      format(Sys.time(), "%Y%m%d-%H%M%OS"),
      digest::digest(data, algo = "md5"),
  ...
  # write out the results as R obj
  save(data, file = file.path(resultsDir, fileName),
            row.names = FALSE, quote = TRUE)
}
        ...
        lapply(
          ...
          function(question) {
            ...
            if (question$type == "text") {
              ...
              input <- radioButtons(ns(question$id), choices = question$choices, inline = question$inline, NULL)
